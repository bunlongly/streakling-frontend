...
export default function MyCardsPage() {
  ...
  useEffect(() => {
    if (!synced) return;
    api.card.listMine().then(r => setCards(r.data)).catch(() => setCards([]));
  }, [synced]);
      ...
      <main className="min-h-dvh bg-brand-mix">
        ...
        <section className="max-w-4xl mx-auto mt-16 p-6 card-surface">
          <div className="flex items-center justify-between">
            ...
            <Link href="/profile/cards/create" className="btn">New Card</Link>
          </div>

          {!synced ? <p className="mt-6 muted">Loading…</p> :
            cards?.length ? (
                  ...
                  <li key={c.id} className="p-4 rounded-md bg-surface/60">
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="flex gap-2 shrink-0">
                        <Link className="btn-outline" href={`/profile/cards/${c.id}`}>Edit</Link>
                        {c.publishStatus === 'PUBLISHED' && (
                          <Link
                            className="btn-outline"
                            href={`/profile/digitalcard/${encodeURIComponent(c.appName)}/${encodeURIComponent(c.slug)}`}
                          >
                            View
                          </Link>
                        )}
